...
public class ClassController {
	...
	@GetMapping("accept")
	public String acceptClass(@RequestParam(value = "classId", required = false) Long classId, Model model) {
		boolean isLogged = loggedService.isLogged();
		if (isLogged) {
			classBatchService.acceptClass(classId, CLASS_STATUS.PLANNED.getStatus());
	...
	@PostMapping("decline")
	public String declineClass(@RequestParam(value = "classId", required = false) Long classId,
			@RequestParam(value = "remarks", required = false) String remarks, Model model) {
		boolean isLogged = loggedService.isLogged();
		if (isLogged) {
	...
	@GetMapping("start")
	public String startClass(@RequestParam(value = "classId", required = false) Long classId, Model model) {
		boolean isLogged = loggedService.isLogged();
		if (isLogged) {
			classBatchService.startClass(classId, CLASS_STATUS.IN_PROCESS.getStatus());
			return "redirect:/class/view/" + classId;
		}
		return "redirect:/login";
	}
